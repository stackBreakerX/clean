...
class AutomationManager @Inject constructor(
    ...
    private suspend fun startService(): AutomationService {
        ...
        if (currentServices.contains(ourServiceComp)) {
            ...
            if (afterToggleOff.contains(ourServiceComp)) throw kotlin.IllegalStateException("Failed to remove our ACS service")
    ...
    private suspend fun stopService() {
        ...
        if (!setupHelper.hasSecureSettings()) {
            throw kotlin.IllegalStateException("stopService(): Trying to stop service but secure settings permission isn't available")
        }
