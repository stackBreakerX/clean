...
class StorageStatsManager2 @Inject constructor(
    ...
    suspend fun queryStatsForAppUid(storageId: StorageId, pkg: Installed): StorageStats = ipcFunnel.use {
        val appUid = pkg.applicationInfo?.uid ?: throw kotlin.IllegalStateException("${pkg.id} is missing an UID")
    ...
    suspend fun getTotalBytes(id: StorageId): Long {
        ...
        if (value == ERRROR_MIN || value == ERROR_MAX) throw kotlin.IllegalStateException("Total bytes is $value")
        return value
    }
